...
const App: React.FC = () => {
  ...
  const sections = useMemo(
    () => [
      ...
      {
        id: "whylocal",
        component: <WhyLocalAI lang={lang} />,
        isLazy: true,
        disableWrapperAnimation: true,
      },
      {
        id: "individuals",
        component: <Individuals lang={lang} />,
        isLazy: true,
        disableWrapperAnimation: true,
      },
      {
        id: "services",
        component: <Services lang={lang} onDiscuss={handleDiscuss} />,
        isLazy: true,
      },
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden transition-colors duration-700 bg-black">
      ...
      <div
        className={`fixed right-8 top-1/2 -translate-x-1/2 z-[100] hidden lg:flex flex-col gap-4 items-center transition-opacity duration-500 ${isChatOpen ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
        {sections.map((_, i) => (
          <button
            key={i}
      ...
      <main ref={containerRef} onScroll={handleScroll} className="snap-container relative z-10">
        {sections.map(section => (
          ...
          >
            {section.isLazy ? (
              <Suspense fallback={<Loader />}>{section.component}</Suspense>
            ) : (
              section.component
            )}
          </SectionWrapper>
...
const SectionWrapper: React.FC<{
  ...
  noPadding?: boolean;
}> = React.memo(({ children, id, disableWrapperAnimation, noPadding }) => {
