...
const FAQ: React.FC<FAQProps> = ({ lang }) => {
  ...
  useEffect(() => {
    if (activeQuestionId !== null) {
      lastActiveIdRef.current = activeQuestionId;
      // eslint-disable-next-line no-undef
      ...
      triggerRefs.current[lastActiveIdRef.current]?.focus();
    }
  }, [activeQuestionId]);
          ...
          {activeQuestionId && activeItem && (
            <>
              ...
              <motion.div
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby={`faq-question-title-${activeItem.id}`}
                initial={{ y: "100%" }}
                animate={{ y: 0 }}
                exit={{ y: "100%" }}
                ...
                <div className="pointer-events-auto w-full max-w-4xl mx-4 mb-4 md:mb-8 bg-[#050816] border border-[#00f2ff]/30 rounded-[32px] overflow-hidden shadow-[0_-20px_80px_rgba(0,0,0,0.8)] relative flex flex-col max-h-[80vh]">
                  ...
                  <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/5">
                    ...
                    <button
                      ref={closeButtonRef}
                      onClick={() => setActiveQuestionId(null)}
                      className="p-2 hover:bg-white/10 rounded-full transition-colors group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00f2ff]"
                    >
                  ...
                  <div className="p-6 md:p-10 overflow-y-auto">
                    <div className="mb-6">
                      <h3 id={`faq-question-title-${activeItem.id}`} className="text-xl md:text-2xl font-bold text-[#00f2ff] mb-2 font-display">
                        {`> ${activeItem.question}`}
                      </h3>
