...
const FAQ: React.FC<FAQProps> = ({ lang }) => {
  ...
  const faqItems = FAQ_DATA[lang], t = UI_TEXT[lang].faq;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => e.key === "Escape" && activeQuestionId && handleClose();
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [activeQuestionId, handleClose]);
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {faqItems.map((item, index) => (
            <motion.button
              ...
              aria-controls={activeQuestionId === item.id ? "faq-drawer" : undefined}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.1 }}
              onClick={() => setActiveQuestionId(item.id)}
              className={`group relative p-8 text-left rounded-3xl border transition-all duration-300 h-full flex flex-col justify-between focus-visible:ring-2 focus-visible:ring-[#00f2ff] outline-none ${
                activeQuestionId === item.id ? "bg-[#00f2ff]/10 border-[#00f2ff] shadow-[0_0_30px_rgba(0,242,255,0.2)]" : "glass border-white/10 hover:border-[#00f2ff]/50 hover:bg-white/5"
              }`}
            >
              {/* Decorative Number */}
              ...
              <h3
                itemProp="name"
                className="text-lg font-bold text-white [.light_&]:text-black pr-8 relative z-10 group-hover:text-[#00f2ff] transition-colors"
              >
                {item.question}
              </h3>
        ...
        <AnimatePresence>
          {activeQuestionId && activeItem && (
            <>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={handleClose} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[150]" />
              <motion.div
                id="faq-drawer" role="dialog" aria-modal="true" aria-labelledby="faq-drawer-title"
                ...
                <div className="pointer-events-auto w-full max-w-4xl mx-4 mb-4 md:mb-8 bg-[#050816] border border-[#00f2ff]/30 rounded-[32px] overflow-hidden shadow-[0_-20px_80px_rgba(0,0,0,0.8)] relative flex flex-col max-h-[80vh]">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-white/5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] font-mono text-white/40 uppercase tracking-widest ml-2">
                        root@architect-ai:~/knowledge-base
                      </span>
                    </div>
                    <button
                      ref={closeButtonRef} onClick={handleClose}
                    ...
                    >
                      ...
                      <XIcon className="w-5 h-5 text-white/60 group-hover:text-white group-hover:rotate-90 transition-all duration-300" />
                    </button>
                  </div>
                  ...
                  <div className="p-6 md:p-10 overflow-y-auto">
                    ...
                    <div className="mt-6 flex justify-end">
                      <button
                        onClick={handleClose}
                        className="px-6 py-3 bg-white/10 hover:bg-white/20 rounded-lg text-xs font-bold uppercase tracking-widest text-white transition-colors focus-visible:ring-2 focus-visible:ring-[#00f2ff] outline-none"
                      >
                        {t.close} [ESC]
                      </button>
                    </div>
