...
const BlogModal: React.FC<BlogModalProps> = ({ isOpen, onClose, lang }) => {
  ...
  const closeLabel = lang === Language.RU ? "Закрыть" : "Close";
            ...
            >
              ...
              <div className="flex justify-between items-center p-8 border-b border-white/5 bg-[#050816] [.light_&]:bg-white z-20">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 id="blog-modal-title" className="font-display font-bold text-2xl text-white [.light_&]:text-black">
                ...
                <button
                  ...
                  className="w-12 h-12 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-colors text-white [.light_&]:text-black focus-visible:ring-2 focus-visible:ring-[#7b2ff7]"
                >
                  <XIcon />
                </button>
              </div>
                ...
                {posts.map(post => (
                  ...
                  >
                    <div className="absolute top-0 right-0 p-6 opacity-50 group-hover:opacity-100 transition-opacity">
                      <svg
                        className="w-6 h-6 text-white/20 group-hover:text-[#7b2ff7] -rotate-45"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2"
                    ...
                    <div className="flex gap-4 items-center mb-4 text-xs font-mono">
                      ...
                      <span className="px-2 py-0.5 rounded border border-white/10 text-white/40">
                        {post.tag}
                      </span>
                      <span className="text-white/30">{post.readTime} read</span>
                    </div>

                    <h4 className="text-xl md:text-2xl font-bold text-white [.light_&]:text-black mb-3 group-hover:text-[#7b2ff7] transition-colors">
                      {post.title}
                    </h4>
                    <p className="text-white/60 [.light_&]:text-black/60 leading-relaxed max-w-2xl">
                      {post.excerpt}
                    </p>
                  </motion.article>
                ))}
