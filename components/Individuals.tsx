        ...
        >
          ...
          {[0, 120, 240].map((deg, i) => (
            <motion.circle
              ...
              animate={{
                cx: [
                  Math.cos((deg * Math.PI) / 180) * 30,
                  Math.cos(((deg + 360) * Math.PI) / 180) * 30,
                ],
                cy: [
                  Math.sin((deg * Math.PI) / 180) * 30,
                  Math.sin(((deg + 360) * Math.PI) / 180) * 30,
                ],
              }}
...
const TutorVisual = () => (
  <div className="relative w-full h-full flex items-center justify-center perspective-[1000px]">
    ...
    >
      ...
      {/* Pages (Stack) */}
      <div
        className="absolute top-2 bottom-2 left-1 w-full bg-white rounded-r transform translate-z-[-5px]"
        style={{ transform: "translateZ(-5px)" }}
      ></div>
      <div
        className="absolute top-3 bottom-3 left-1 w-[98%] bg-gray-200 rounded-r transform translate-z-[-10px]"
        style={{ transform: "translateZ(-10px)" }}
      ></div>

      {/* Back Cover */}
      <div
        className="absolute inset-0 bg-[#0f172a] rounded-r-lg transform translate-z-[-15px]"
        style={{ transform: "translateZ(-15px)" }}
      ></div>

      {/* Floating Formulas */}
      <motion.div
        className="absolute -top-10 -right-10 text-3xl font-mono text-[#00f2ff] font-bold"
      ...
      <motion.div
        className="absolute top-1/2 -left-16 text-2xl font-mono text-[#7b2ff7] font-bold"
      ...
      <motion.div
        className="absolute -bottom-5 -right-5 text-xl font-mono text-white font-bold"
        animate={{ y: 15, opacity: [0, 1, 0] }}
  ...
  <div className="relative w-full h-full flex items-center justify-center">
    ...
    >
      ...
      <div className="mt-auto pt-2 relative">
        ...
        <span className="text-[6px] uppercase tracking-widest font-bold text-gray-500">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap gap-2 mt-6 md:mt-8">
              {item.features?.map((f: string, i: number) => (
                <span
                  key={i}
                  className="px-3 py-1 rounded-lg bg-white/5 text-[10px] md:text-xs text-white/50 border border-white/5 flex items-center gap-2 group cursor-default hover:bg-white/10 transition-colors"
                >
