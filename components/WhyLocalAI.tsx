  ...
  <div className="relative w-full h-full flex items-center justify-center p-4">
    ...
    <div className="relative w-full h-full flex gap-4">
      ...
      <div className="flex-1 rounded-2xl bg-red-500/5 border border-red-500/20 flex flex-col items-center justify-center relative overflow-hidden group">
        ...
        <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border border-red-500/30 flex items-center justify-center mb-4 text-red-500">
          <svg
            ...
            strokeWidth="1.5"
          >
            <path d="M17.5 19c0-1.7-1.3-3-3-3h-11c-1.7 0-3 1.3-3 3 .6 1.7 2.3 3 4 3h10c1.7 0 3.4-1.3 4-3z" />
            <path d="M17.5 19c1.7 0 3-1.3 3-3 0-1.1-.6-2-1.5-2.6.4-.8.5-1.7.2-2.6-.5-1.6-2-2.7-3.7-2.7-1.3 0-2.5.7-3.2 1.8C11.8 8.8 10.5 8 9 8c-2.2 0-4 1.8-4 4 0 .4.1.8.2 1.2" />
          </svg>
        </div>
        <div className="text-red-400 font-mono text-xs uppercase tracking-widest">
          {UI_TEXT[lang].visuals.unsafe}
        </div>
        {/* Padlock Open */}
        <svg
          className="mt-4 text-red-500/50 w-8 h-8"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
      ...
      <div className="flex-1 rounded-2xl bg-[#00f2ff]/5 border border-[#00f2ff]/20 flex flex-col items-center justify-center relative overflow-hidden group">
        ...
        <div className="text-[#00f2ff] font-mono text-xs uppercase tracking-widest">
          {UI_TEXT[lang].visuals.sovereign}
        </div>
        {/* Padlock Closed */}
        <svg
          className="mt-4 text-[#00f2ff]/50 w-8 h-8"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
      ...
      {/* VS Badge */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black border border-white/20 flex items-center justify-center z-10 font-black text-xs italic">
        VS
      </div>
    </div>
  </div>
  ...
  <div className="relative w-full h-full flex items-center justify-center">
    ...
    <div className="w-48 h-64 border border-white/10 rounded-lg p-2 flex flex-col gap-2 relative bg-[#050816] [.light_&]:bg-gray-100">
      {[1, 2, 3, 4].map(i => (
        <div
          key={i}
          className="flex-1 border border-white/5 rounded bg-white/5 flex items-center px-3 gap-2"
        >
      ...
      >
        <div
          className="absolute inset-0 border border-[#7b2ff7] opacity-50 bg-[#7b2ff7]/10 translate-z-8"
        ...
        ></div>
        <div
          className="absolute inset-0 border border-[#7b2ff7] opacity-50 bg-[#7b2ff7]/10 rotate-y-90"
...
const HardwareVisual = () => (
  <div className="relative w-full h-full flex items-center justify-center">
    <div className="grid grid-cols-2 gap-4">
      <motion.div
        className="w-24 h-32 border border-white/20 rounded-lg bg-white/5 relative p-2"
...
const getVisual = (type: string, lang: Language) => {
  switch (type) {
    case "comparison":
      return <ComparisonVisual lang={lang} />;
    case "database":
      return <DatabaseVisual />;
    case "graph":
      return <ProductivityVisual lang={lang} />;
    case "hardware":
            ...
            {item.visualType === "comparison" && (
              <div className="space-y-4">
                {item.features.map((feat: any, i: number) => (
                  <div
                    key={i}
                    className="grid grid-cols-3 gap-4 text-xs md:text-sm border-b border-white/5 pb-2"
                  >
                    <div className="text-white/40 font-bold uppercase tracking-wider self-center">
                      {feat.name}
                    </div>
            ...
            {item.visualType !== "comparison" && item.stats && (
              <div className="grid grid-cols-2 gap-4 md:gap-6 mt-4">
                {item.stats.map((stat: any, i: number) => (
                  <div key={i}>
                    <div className="text-2xl md:text-4xl font-mono font-bold text-white [.light_&]:text-black">
                      {stat.value}
                    </div>
            ...
            {item.visualType !== "comparison" && item.features && (
              <div className="flex flex-wrap gap-2 mt-6 md:mt-8">
                {item.features.map((f: string, i: number) => (
                  <span
                    key={i}
                    className="px-3 py-1 rounded-lg bg-white/5 text-[10px] md:text-xs text-white/50 border border-white/5"
                  >
                    {f}
                  </span>
                ))}
