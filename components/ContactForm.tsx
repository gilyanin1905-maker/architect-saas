...
const ContactForm: React.FC<ContactFormProps> = ({ lang }) => {
  ...
  const successHeaderRef = useRef<HTMLHeadingElement>(null);
  const prevStatusRef = useRef<"idle" | "loading" | "success" | "error">("idle");

  useEffect(() => {
    if (status === "success" && successHeaderRef.current) {
      successHeaderRef.current.focus();
    } else if (status === "idle" && prevStatusRef.current === "success" && nameInputRef.current) {
                ...
                <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                  <div className="space-y-2">
                    <label
                      ...
                      className="text-xs font-bold text-white/40 uppercase ml-1 block cursor-pointer"
                    >
                      {t.name}
                    </label>
                    <input
