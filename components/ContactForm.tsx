...
const ContactForm: React.FC<ContactFormProps> = ({ lang }) => {
  ...
  const handleSendAgain = () => {
    setStatus("idle");
    // Use a small delay to ensure the form is rendered before attempting focus
    setTimeout(() => {
    ...
    <section id="contact" className="py-12 md:py-24 relative overflow-hidden">
      <div className="container mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-10 md:gap-20">
          ...
          <div
            className="glass p-6 md:p-10 lg:p-12 rounded-[32px] md:rounded-[40px] relative"
            aria-live="polite"
          >
            {status === "success" ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                ...
                >
                  {status === "loading" ? (
                    <div
                      className="w-6 h-6 border-2 border-black/30 border-t-black rounded-full animate-spin mx-auto"
                      aria-hidden="true"
                    ></div>
                  ) : (
                    t.submit
