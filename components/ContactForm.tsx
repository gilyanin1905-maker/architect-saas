...
const ContactForm: React.FC<ContactFormProps> = ({ lang }) => {
  ...
  useEffect(() => {
    if (prevStatusRef.current === "success" && status === "idle") {
                ...
                >
                  {status === "loading" ? (
                    <>
                      ...
                      <div className="w-6 h-6 border-2 border-black/30 border-t-black rounded-full animate-spin mx-auto" aria-hidden="true" />
                    </>
                  ) : (
                    t.submit
                  )}
