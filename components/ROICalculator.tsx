...
const ROICalculator: React.FC<ROICalculatorProps> = ({ lang }) => {
  ...
  const savingsPercent = Math.max(
    0,
    Math.min(100, Math.round((monthlySavings / monthlyCloudCost) * 100))
  );
        ...
        <div className="glass rounded-[32px] md:rounded-[40px] p-6 md:p-12 lg:p-20 overflow-hidden relative transition-all duration-700">
          ...
          <div className="grid lg:grid-cols-2 gap-8 md:gap-20 items-start">
            ...
            <div className="space-y-8">
              <div>
                <div className="text-[8px] md:text-[10px] font-mono font-bold uppercase tracking-[0.5em] text-[#00f2ff] mb-2 md:mb-4">
                  {t.label}
                </div>
              ...
              <div className="p-6 bg-black/20 rounded-2xl border border-white/5">
                <h4 className="text-xs font-bold text-white/60 mb-6 uppercase tracking-widest">
                  {t.chartTitle}
                </h4>
                <div className="relative h-48 w-full flex items-end justify-between gap-2 pl-8 pb-8 border-l border-b border-white/10">
                  {/* Y-Axis Label */}
                  <div className="absolute left-0 top-0 -rotate-90 origin-top-left text-[9px] text-white/30 tracking-widest">
                    COST ($)
                  </div>
                  ...
                  <div className="w-full h-full relative">
                    ...
                    <svg className="absolute inset-0 w-full h-full overflow-visible">
                      ...
                      />
                      <text x="350" y="15" fill="#ef4444" fontSize="10">
                        Cloud (∞)
                      </text>
                      ...
                      />
                      <text x="350" y="170" fill="#00f2ff" fontSize="10">
                        Local (Flat)
                      </text>
                    </svg>
                    ...
                    <motion.div
                      ...
                      transition={{ delay: 2 }}
                    >
                      <div className="absolute top-4 -left-4 w-max text-[9px] bg-white/10 backdrop-blur px-2 py-1 rounded text-white">
                        ROI Point
                      </div>
                    </motion.div>
                  </div>
                  ...
                  {/* X-Axis Label */}
                  <div className="absolute bottom-[-20px] right-0 text-[9px] text-white/30 tracking-widest">
                    TIME (MONTHS)
                  </div>
                </div>
              ...
              <div className="space-y-4 relative z-10">
                <div className="flex justify-between text-[11px] py-4 border-b border-white/5">
                  ...
                  <span className="font-mono text-white/70">
                    ${Math.round(monthlyCloudCost).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between text-[11px] py-4 border-b border-white/5">
