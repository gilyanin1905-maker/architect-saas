...
// --- Buy Modal Component ---
const BuyModal: React.FC<{ isOpen: boolean; onClose: () => void; lang: Language }> = ({
  isOpen,
  onClose,
  lang,
}) => {
          ...
          >
            <div className="bg-[#0e1621] border border-white/10 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-3">
                <a
                  href="https://t.me/Architect_SaaS"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-[#0088cc]/20 border border-white/5 hover:border-[#0088cc]/50 transition-all group"
                >
                  <div className="text-[#0088cc]">
                    <TelegramIcon className="w-6 h-6" />
                  </div>
                  <span className="font-bold text-white group-hover:text-[#00f2ff]">
                    {t.telegram}
                  </span>
                </a>
                <a
                  href="#"
                  className="flex items-center gap-4 p-4 rounded-xl bg-white/5 hover:bg-purple-500/20 border border-white/5 hover:border-purple-500/50 transition-all group opacity-50 cursor-not-allowed"
                >
                ...
                <a
                  href="#contact"
                  onClick={onClose}
                  className="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-[#00f2ff]/10 to-[#7b2ff7]/10 hover:from-[#00f2ff]/20 hover:to-[#7b2ff7]/20 border border-white/10 transition-all group"
                >
...
const ServiceCard: React.FC<{
  service: (typeof SERVICES_DATA)["RU"][0];
  lang: Language;
  onDiscuss: (title: string) => void;
}> = ({ service, lang, onDiscuss }) => {
  ...
  return (
    <>
      ...
      >
        {/* Top Gradient Glow */}
        <div
          className={`absolute top-0 left-0 w-full h-[150px] bg-gradient-to-b ${service.gradient} opacity-20 group-hover:opacity-30 transition-opacity duration-500`}
        ></div>
        ...
        <div className="relative z-10 p-6">
          ...
          <div
            className={`text-xl md:text-2xl font-display font-bold bg-gradient-to-r ${service.gradient} bg-clip-text text-transparent`}
          >
            {service.role}
          </div>
        </div>
        ...
        <div className="relative z-10 px-6 flex-1 flex flex-col">
          ...
          <ul className="space-y-3 mt-auto mb-6">
            {service.features.map((feature, i) => (
              <li
                key={i}
                className="text-[10px] md:text-xs font-mono text-white/50 flex items-center gap-2"
              >
                <span
...
const Services: React.FC<ServicesProps> = ({ lang, onDiscuss }) => {
  ...
  return (
    <section
      id="services"
      className="py-8 md:py-32 relative w-full overflow-hidden flex flex-col justify-center"
    >
      {/* Background Ambience */}
