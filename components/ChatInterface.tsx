...
const SITE_URL = import.meta.env.VITE_SITE_URL || "https://architect-saas.dev";
const SITE_NAME = import.meta.env.VITE_SITE_NAME || "Architect SaaS";
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ isOpen, onClose, lang, triggerMessage }) => {
  ...
  const handleSendMessage = async (e?: FormEvent, forcedText?: string) => {
    ...
    if (editingMessageId && !forcedText) {
      ...
      try {
        ...
        const contextMessages = [...history, editedMsg]
    ...
    const replyContext = replyContextMessage
      ? `[Context: User replying to message "${replyContextMessage.content}"]\n\n`
      : "";
  ...
  const fetchAIResponse = async (messages: any[], sessionId: string) => {
    try {
      ...
      console.warn(
        "API Request Failed (likely due to invalid key or network issue). Falling back to offline mode.",
        error
      );
      ...
      setTimeout(() => {
        ...
        setSessions(prev =>
          prev.map(s => (s.id === sessionId ? { ...s, messages: [...s.messages, fallbackMsg] } : s))
        );
      }, 1500);
    }
  };
        ...
        >
          ...
          <div className="flex-1 flex flex-col bg-[#0e1621] relative w-full h-full">
            ...
            >
              ...
              <div
                className="absolute inset-0 opacity-[0.03] pointer-events-none sticky top-0"
                style={{
                  ...
                  return (
                    <React.Fragment key={msg.id}>
                      ...
                      >
                        ...
                        <div
                          className={`absolute top-0 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 p-1 rounded-lg bg-[#17212b] border border-white/10 shadow-lg z-10
                        ...
                        <div
                          className={`relative max-w-[85%] md:max-w-[70%] min-w-[100px] shadow-sm rounded-2xl text-[15px] leading-relaxed break-words
                                                    ${
                                                      isUser
                        ...
                        >
                              ...
                              {msg.content.split("```").map((part, i) => {
                                if (i % 2 === 1) {
                                  return (
                                    <div
                                      key={i}
                                      className="my-2 p-3 bg-black/30 rounded-lg text-xs font-mono overflow-x-auto border border-white/5 text-[#00f2ff] whitespace-pre"
                                    >
                                      {part}
                                    </div>
                                  );
                          ...
                          <div
                            className={`flex items-center justify-end gap-1 px-4 pb-2 text-[10px] ${isUser ? "text-[#7faedb]" : "text-[#6c7883]"}`}
                          >
                    ...
                    <div className="flex items-center gap-3 py-2 pl-2 overflow-hidden w-full">
                      ...
                      <div className="flex flex-col border-l-2 border-[#00f2ff] pl-2 overflow-hidden w-full">
                        <span className="text-[#00f2ff] text-xs font-bold">
                          {editingMessageId
                            ? "Editing Message"
                            : `Reply to ${replyContextMessage?.role === "user" ? "You" : "Eva"}`}
                        </span>
                        <span className="text-white/50 text-xs truncate w-full pr-4">
                          {editingMessageId
                            ? "Edit to regenerate response"
